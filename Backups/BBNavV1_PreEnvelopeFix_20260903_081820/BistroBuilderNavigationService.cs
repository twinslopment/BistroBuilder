...
public sealed partial class BistroBuilderNavigationService : MonoBehaviour
{
    ...
    private readonly Dictionary<string, DestinationReservation> destinations =
        new Dictionary<string, DestinationReservation>(StringComparer.Ordinal);
        ...
        out BistroBuilderNavigationRouteKind routeKind)
    {
        ...
        // NavMesh es el sustrato geométrico normal. El grid histórico queda
        // relegado a recuperación degradada y no se calcula en paralelo.
        ...
        out BistroBuilderNavigationRoute route)
    {
        ...
        if (!TryBuildRoute(requesterId, agent, origin, destination, route.points,
        ...
        Vector3 destination)
    {
        ...
        return TryBuildRoute(requesterId, agent, origin, destination, points,
        ...
        out float congestion)
    {
        ...
        for (int i = 1; i < navPath.corners.Length; i++)
        {
            ...
            if (!SegmentAllowedForNavMesh(previous, corner, defaultAgentRadius,
        ...
        {
            if (!SegmentAllowedForNavMesh(previous, destination, defaultAgentRadius,
                    agent, requesterId, origin, destination))
            {
                points.Clear();
                return false;
        ...
        out float congestion)
    {
        ...
        if (Vector3.Distance(origin, destination) <= gridCellSize ||
            SegmentAllowed(origin, destination, defaultAgentRadius,
                agent, requesterId, origin, destination))
        {
            points.Add(destination);
        ...
        while (heap.Count > 0 && expanded < maximumExpandedGridNodes)
        {
            ...
            for (int n = 0; n < 8; n++)
            {
                ...
                if (!isGoal && !IsPointTraversable(world, defaultAgentRadius,
                        agent, requesterId, origin, destination, false))
                    continue;
                if (!SegmentAllowed(currentWorld, world, defaultAgentRadius,
                        agent, requesterId, origin, destination))
                    continue;
        ...
        reverse.Reverse();
        SmoothGridRoute(origin, destination, reverse, requesterId, agent, points);
        if (points.Count == 0 ||
        ...
        while (index < raw.Count)
        {
            ...
            for (int test = raw.Count - 1; test >= index; test--)
            {
                Vector3 target = test == raw.Count - 1
                    ? destination
                    : raw[test];
                if (SegmentAllowed(anchor, target, defaultAgentRadius,
                        agent, requesterId, origin, destination))
                {
                    best = test;
                    break;
    ...
    private bool IsDestinationOccupied(
        ...
        int priority)
    {
        foreach (KeyValuePair<string, DestinationReservation> pair in destinations)
        {
            if (string.Equals(pair.Key, ownerId, StringComparison.Ordinal)) continue;
            DestinationReservation other = pair.Value;
            if (other == null || other.expiresAt <= Time.unscaledTime) continue;
            float limit = radius + other.radius;
            if (HorizontalDistanceSquared(point, other.position) < limit * limit &&
        ...
        {
            if (string.Equals(pair.Key, ownerId, StringComparison.Ordinal)) continue;
            AgentPresence other = pair.Value;
            if (other == null ||
                Time.unscaledTime - other.seenAt > presenceStaleSeconds) continue;
            float limit = radius + other.radius;
            if (HorizontalDistanceSquared(point, other.position) < limit * limit)
                return true;
        }
    ...
    private void CommitDestination(
        ...
        int priority)
    {
        destinations[ownerId] = new DestinationReservation
        {
            ownerId = ownerId,
            position = point,
            radius = radius,
            priority = priority,
            expiresAt = Time.unscaledTime + destinationReservationSeconds
        };
    }
    ...
    private void CleanupTransientState()
    {
        ...
        var staleDestinations = new List<string>();
        foreach (KeyValuePair<string, DestinationReservation> pair in destinations)
        {
            if (pair.Value == null || pair.Value.expiresAt <= now)
                staleDestinations.Add(pair.Key);
        }
        for (int i = 0; i < staleDestinations.Count; i++)
            destinations.Remove(staleDestinations[i]);
        dynamicEnvelopes.RemoveAll(item => item == null);
    }
