...
[V1 REDISEÑADO — FASE K / CHAIR FUNCTIONAL AUTHORING] La publicación de una silla deja de depender de que el particionado semántico detallado sea automation-ready. La semántica por triángulos sigue siendo una capa de enriquecimiento: cuando es fiable, alimenta colliders detallados; cuando no lo es, no bloquea una silla cuya identidad, geometría funcional, asiento, orientación y dimensiones sí están validadas.

[V1 IMPLEMENTADO — FASE K] `SavicChairAuthoringPlanner` ya no usa el score de clasificación ni `semanticParts.automationReady` como gates de autoría. La identidad `Chair` la decide el clasificador; la readiness funcional la deciden exclusivamente el perfil geométrico de silla, seat height, escala, orientación, dimensiones, skinned/negative-scale y los validadores posteriores.

[V1 IMPLEMENTADO — FASE K] `SavicChairColliderBuilder` dispone de dos estrategias canónicas. Si existe semántica detallada fiable, conserva `SEMANTIC_SEAT_BACK_SUPPORTS[_ARMS]`. Si no, genera `GEOMETRY_ENVELOPE_SEAT_BACK_SUPPORT`: tres volúmenes conservadores derivados de dimensiones finales, seat height y orientación canónica. Esta ruta no inventa partes del mesh y sigue pasando el mismo validador de volumen, seat height y topología.
