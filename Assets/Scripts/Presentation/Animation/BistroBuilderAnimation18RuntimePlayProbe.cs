...
/// SÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â³lo existe en Editor y nunca entra en builds del juego.
/// </summary>
[DisallowMultipleComponent]
public sealed class BistroBuilderAnimation18RuntimePlayProbe : MonoBehaviour
{
    ...
    {
        BistroBuilderAnimation18RuntimePlayProbe[] probes =
    ...
    {
        // Espera a que el bootstrap general estabilice las autoridades de escena.
        // El probe no debe capturar una instancia transitoria antes de Start.
    ...
    private IEnumerator RunSeatSit()
    {
        ...
        Vector3 seatDelta = actor.transform.position - seat.SeatPoint.position;
        seatDelta.y = 0f;
        if (seatDelta.sqrMagnitude > 0.0004f)
            throw new InvalidOperationException(
                "Actor no quedo reconciliado horizontalmente al SeatFrame.");
    }
    ...
    private IEnumerator RunSeatStand()
    {
        ...
        Vector3 exitDelta = actor.transform.position -
            seat.CustomerApproachPoint.position;
        exitDelta.y = 0f;
        if (exitDelta.sqrMagnitude > 0.0004f)
            throw new InvalidOperationException(
                "Actor no quedo reconciliado horizontalmente al ExitFrame.");
    ...
    private IEnumerator RunPortal()
    {
        ...
        doorEnvelope.ConfigureForEditor(
            Vector3.zero,
        ...
        float timeoutSeconds)
    {
        ...
        if (!handle.IsTerminal)
            throw new TimeoutException(
                "Interaccion supero watchdog: " + handle.InteractionId + "; Status=" + handle.Status + "; Phase=" + handle.Phase + "; Commit=" + handle.CommitState);
    }
    ...
    private static void FinishAndExit(bool success, string message)
    {
        ...
        File.WriteAllText(
            Path.GetFullPath("Animation18PlayModeReport.txt"),
            report);
