...
public sealed class BistroBuilderConstructionPlayerPanel : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        {
            BistroBuilderInteractionSurface.Attach(entry.Value)?.SetSelected(entry.Key == tool.Mode);
        ...
        initialControls.anchoredPosition = new Vector2(-12, furnitureMode ? 438 : 200);
    ...
    private void Build()
    {
        ...
        Zone(otherZones, "Baño", "zone.bathroom"); Zone(otherZones, "Barra", "zone.bar");
        Zone(catalogue, "Terraza", "zone.terrace");
    ...
    private static TMP_Text Text(Transform parent, string value, int size, float height)
    {
        ...
        var role = size >= 24 ? BistroBuilderUiStyleRole.Heading : size >= 18 ? BistroBuilderUiStyleRole.Subheading : size <= 12 ? BistroBuilderUiStyleRole.Caption : BistroBuilderUiStyleRole.Body;
        text.gameObject.AddComponent<BistroBuilderUiStyleTag>().Configure(role);
        BistroBuilderTypography.Apply(text, role);
