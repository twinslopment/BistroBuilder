...
/// Es Presentation pura; no decide ningÃƒÆ’Ã‚Âºn estado de negocio.
/// </summary>
...
public sealed class BistroBuilderUiDesignSystem : MonoBehaviour
{
    ...
    public bool ValidateConfiguration(out string error)
    {
        ...
        if (canvas == null)
        {
            error = "BB UI Design System debe vivir bajo el Canvas HUD canÃƒÆ’Ã‚Â³nico.";
            return false;
        }
    ...
    private BistroBuilderUiStyleRole ResolveTextRole(GameObject target, string value)
    {
        ...
        if (ContainsAny(key, "crÃƒÆ’Ã‚Â­tico", "critico", "bloquead", "error")) return BistroBuilderUiStyleRole.StatusCritical;
        if (ContainsAny(key, "atenciÃƒÆ’Ã‚Â³n", "atencion", "saturad", "espera")) return BistroBuilderUiStyleRole.StatusAttention;
