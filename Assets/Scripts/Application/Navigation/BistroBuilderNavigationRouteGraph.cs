...
public sealed class BistroBuilderNavigationRouteGraph
{
    ...
    public bool TryFindGateAnchors(
        ...
        Func<Vector3, Vector3, float> guidancePenalty,
        ...
        while (open.Count > 0)
        {
            ...
            for (int i = 0; i < outgoing.Count; i++)
            {
                ...
                float congestion = congestionPenalty != null
                    ...
                    : 0f;
                Vector3 currentPosition = nodes.TryGetValue(authoritative.nodeId, out Node currentNode) && currentNode != null
                    ? currentNode.position : next.position;
                float guidance = guidancePenalty != null
                    ? Mathf.Max(0f, guidancePenalty(currentPosition, next.position))
                    : 0f;
