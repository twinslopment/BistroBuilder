...
/// <summary>
/// Adaptador de rutas de camareros. Bloque 13 sigue decidiendo tareas y orden;
/// Bloque 17 es ahora la autoridad fisica de por donde debe circular.
/// </summary>
    ...
    IBistroBuilderWaiterRouteProvider
{
        ...
        out float lengthMeters)
    {
        ...
        {
            LastRouteKind = kind == BistroBuilderNavigationRouteKind.NavMesh
                ? BistroBuilderWaiterRouteKind.NavMeshOptimal
                : BistroBuilderWaiterRouteKind.ExternalProfessional;
            return true;
        }
    ...
    public float EstimateRouteMeters(Vector3 origin, Vector3 destination)
    {
        if (navigationService != null)
        {
            float estimate = navigationService.EstimateRouteMeters(
                "waiter-routing-estimate",
                BistroBuilderNavigationAgentMask.Waiter,
                origin,
                destination);
            if (!float.IsInfinity(estimate)) return estimate;
        }
