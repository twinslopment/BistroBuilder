...
public static class BistroBuilderScreenWalkthrough
{
    ...
    static void Log(string message, string stack, LogType type)
    {
        ...
        if (!string.IsNullOrEmpty(stack) &&
            (stack.Contains("RestaurantPlacementValidationService.cs") || stack.Contains("UnityEditor.Search"))) return;
        failure = message;
    }
    ...
    static void BuildSteps()
    {
        ...
        {
            string screen = name;
            Screen(screen, () => Click("BBNav_" + screen), () => {
                if (screen == "Actividad") return;
                if (shell.HasManagementScreenOpen) return;
                if (screen == "Proveedores")
                {
                    var suppliers = UnityEngine.Object.FindFirstObjectByType<BistroBuilderSupplierPlayerRuntimeView>();
                    string error = string.Empty;
                    bool opened = suppliers != null && suppliers.TryOpenFromInterface(out error);
                    string supplierError = opened ? "opened on retry" : (suppliers == null ? "view missing" : error);
                    throw new Exception(screen + " did not open: " + supplierError);
                }
                throw new Exception(screen + " did not open");
            }, null);
            steps.Add(() => {
