...
public static class BistroBuilderFinance3HInstaller
{
    [MenuItem(
        "Tools/Bistro Builder/Finanzas/3H - Instalar + validar + autotest",
        false,
        3070)]
    private static void InstallValidateAndTest()
    {
        if (EditorApplication.isPlayingOrWillChangePlaymode)
        {
            EditorUtility.DisplayDialog(
                "Bistro Builder",
                "Sal de Play Mode antes de instalar 3H.",
                "Aceptar");
            return;
        }
            ...
            string.IsNullOrWhiteSpace(scene.path))
        {
            EditorUtility.DisplayDialog(
                "Bistro Builder",
                "Abre y guarda Prototype_Restaurant antes de instalar 3H.",
                "Aceptar");
            return;
        }
        ...
        if (scene.isDirty)
        {
            EditorUtility.DisplayDialog(
                "Bistro Builder",
                "Guarda la escena antes de instalar 3H.",
                "Aceptar");
            return;
        }
        ...
        Undo.SetCurrentGroupName("Instalar 3H Históricos e informes financieros");
        int undoGroup = Undo.GetCurrentGroup();

        try
        {
            BistroBuilderFinancialResultsService results =
                FindSingleSceneComponent<BistroBuilderFinancialResultsService>(scene);
            ...
            BistroBuilderFinancialHistoryService history =
                GetOrAdd<BistroBuilderFinancialHistoryService>(gameSystems);
            SetReference(history, "financialResultsService", results);
            SetReference(history, "generalGameStateService", generalState);
            ...
            if (!EditorSceneManager.SaveScene(scene))
            {
                throw new InvalidOperationException(
                    "Unity no pudo guardar la escena tras instalar 3H.");
            }

            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();

            bool validationOk = BistroBuilderFinance3HValidator.ValidateCurrentScene(
                out int validationPassed,
                out int validationFailed,
                out string validationReport);
            bool testOk = BistroBuilderFinance3HSelfTest.Run(
                out int testPassed,
                out int testFailed,
            ...
            {
                throw new InvalidOperationException(
                    "La validación automática de 3H no fue limpia. " +
            ...
            EditorUtility.DisplayDialog(
                "Bistro Builder — 3H",
                "3H instalado y probado correctamente." +
        ...
        catch (Exception exception)
        {
            ...
            EditorUtility.DisplayDialog(
                "Bistro Builder",
                "La instalación 3H falló y la escena fue restaurada.\n\n" +
                exception.Message,
                "Aceptar");
        }
        finally
