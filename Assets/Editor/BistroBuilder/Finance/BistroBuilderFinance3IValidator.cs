...
public static class BistroBuilderFinance3IValidator
{
        ...
        out string report)
    {
        ...
        builder.AppendLine("Bistro Builder — Validación 3I");

        Scene scene = SceneManager.GetActiveScene();
        Check(scene.IsValid() && scene.isLoaded,
            "Escena activa válida", ref passed, ref failed, builder);

        BistroBuilderFinanceService[] finance = FindScene<BistroBuilderFinanceService>(scene);
        ...
        Check(finance.Length == 1, "Una autoridad de caja 3A", ref passed, ref failed, builder);
        Check(supplier.Length == 1, "Un puente de compromisos 3C", ref passed, ref failed, builder);
        Check(history.Length == 1, "Un histórico financiero 3H", ref passed, ref failed, builder);
        Check(general.Length == 1, "Un calendario canónico", ref passed, ref failed, builder);
        Check(clocks.Length == 1, "Un reloj canónico", ref passed, ref failed, builder);
        Check(financing.Length == 1, "Una autoridad de financiación 3I", ref passed, ref failed, builder);
        Check(providers.Length == 1, "Un proveedor Save de financiación", ref passed, ref failed, builder);
        ...
        if (financing.Length == 1)
        {
            ...
            Check(finance.Length == 1 && ReferenceEquals(service.FinanceService, finance[0]),
                "3I usa la caja canónica 3A", ref passed, ref failed, builder);
            Check(supplier.Length == 1 && ReferenceEquals(service.SupplierFinanceBridge, supplier[0]),
                "3I usa compromisos canónicos 3C", ref passed, ref failed, builder);
            Check(history.Length == 1 && ReferenceEquals(service.FinancialHistoryService, history[0]),
                "3I usa históricos 3H", ref passed, ref failed, builder);
            Check(general.Length == 1 && ReferenceEquals(service.GeneralGameStateService, general[0]),
        ...
        if (providers.Length == 1)
        {
            ...
            Check(providers[0].SectionId == BistroBuilderFinancingSnapshot.CurrentSchemaId &&
                  providers[0].SectionVersion == BistroBuilderFinancingSnapshot.CurrentSchemaVersion,
                "Contrato Save finance.financing.runtime v1", ref passed, ref failed, builder);
        }
        ...
        Check(sameSectionProviders == 1,
            "Sin sección Save financiera duplicada", ref passed, ref failed, builder);
