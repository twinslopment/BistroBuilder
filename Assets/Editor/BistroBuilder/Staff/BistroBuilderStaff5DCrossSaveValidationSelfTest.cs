...
///
/// Demuestra que la prevalidación universal de Load solo exige estructura
/// autosuficiente de staff.schedule y que el cruce EmployeeId se realiza contra
/// staff.state objetivo durante Apply, no contra la plantilla actualmente abierta.
/// </summary>
public static class BistroBuilderStaff5DCrossSaveValidationSelfTest
{
    [MenuItem(
        "Tools/Bistro Builder/Personal/5D - Autotest Save cruzado horarios",
        false,
        3278)]
    private static void RunMenu()
    {
        ...
        EditorUtility.DisplayDialog(
            "Bistro Builder — 5D Save cruzado",
    ...
    public static bool Run(out int passed, out int failed, out string report)
    {
        ...
        try
        {
            ...
            Check(
                BistroBuilderStaffScheduleEngine.TryValidateStructure(
                    scheduleB,
                    out string structureError),
                "La prevalidación estructural acepta un horario válido sin consultar la partida abierta. " +
                structureError,
                ref passed,
                ref failed,
                log);
            ...
            Check(
                !BistroBuilderStaffScheduleEngine.TryValidateSnapshot(
                    ...
                    staffA,
                    out _),
                "El mismo horario no se cruza contra staff.state de la partida A.",
                ref passed,
                ref failed,
                log);
            ...
            Check(
                ...
                "Tras aplicar staff.state B, el horario B cruza correctamente su EmployeeId. " +
                targetError,
                ref passed,
                ref failed,
                log);
            ...
            Check(
                !BistroBuilderStaffScheduleEngine.TryValidateStructure(
                    duplicated,
                    out _),
                "La prevalidación sigue rechazando duplicados estructurales antes de Apply.",
                ref passed,
                ref failed,
                log);
            ...
            Check(
                !BistroBuilderStaffScheduleEngine.TryValidateStructure(
                    broken,
                    out _),
                "La prevalidación sigue rechazando EmployeeId estructuralmente inválidos.",
                ref passed,
                ref failed,
                log);
        }
