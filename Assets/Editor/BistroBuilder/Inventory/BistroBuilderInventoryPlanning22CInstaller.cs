...
/// <summary>
/// Instalador acumulativo, idempotente y transaccional de 2.2C.
...
/// </summary>
public static class BistroBuilderInventoryPlanning22CInstaller
{
    private const string MenuPath =
        "Tools/Bistro Builder/Inventory/Install or Repair 2.2C Minimum Stock, Alerts and Basic Forecast";

    public const string UiRootName = "BB_2_2C_InventoryPlanningUI";
    ...
    private static void InstallOrRepair()
    {
        if (EditorApplication.isPlayingOrWillChangePlaymode)
        {
            EditorUtility.DisplayDialog(
                "Bistro Builder",
                "Sal de Play Mode antes de instalar 2.2C.",
                "Aceptar"
            );
            return;
        }
            ...
            string.IsNullOrWhiteSpace(scene.path))
        {
            EditorUtility.DisplayDialog(
                "Bistro Builder",
                "Abre y guarda la escena principal antes de instalar 2.2C.",
                "Aceptar"
            );
            return;
        }
        ...
        if (scene.isDirty)
        {
            EditorUtility.DisplayDialog(
                "Bistro Builder",
                "Guarda la escena antes de ejecutar el instalador 2.2C.",
                "Aceptar"
            );
            return;
        }
        ...
        try
        {
            ...
            Undo.RegisterCompleteObjectUndo(
                gameSystems,
                "Instalar planificación de inventario 2.2C"
            );

            BistroBuilderInventoryPlanningService planning =
                GetOrAdd<BistroBuilderInventoryPlanningService>(gameSystems);
            ...
            BistroBuilderInventoryPolicySaveSectionProvider provider =
                GetOrAdd<BistroBuilderInventoryPolicySaveSectionProvider>(
                    gameSystems
                );
            SetReference(provider, "saveGameService", saveService);
            SetReference(provider, "planningService", planning);
            ...
            BistroBuilderInventoryPlanningRuntimeView view =
                GetOrAdd<BistroBuilderInventoryPlanningRuntimeView>(
                    uiRoot.gameObject
                );
            SetReference(view, "planningService", planning);
            SetReference(
                view,
                "cameraController",
            ...
            if (!planning.ValidateConfiguration(out error) ||
            ...
            EditorUtility.SetDirty(planning);
            ...
            Debug.Log(
                "BISTRO BUILDER - 2.2C INSTALADO\n" + result.BuildReport()
            );
            EditorUtility.DisplayDialog(
                "Bistro Builder",
                "2.2C instalado correctamente.\n\n" +
        ...
        catch (Exception exception)
        {
            ...
            EditorUtility.DisplayDialog(
                "Bistro Builder",
                "La instalación 2.2C falló y la escena fue restaurada.\n\n" +
                exception.Message,
                "Aceptar"
            );
        }
    ...
    private static RectTransform FindOrCreateUiRoot(Transform canvas)
    {
        ...
        Undo.RegisterCreatedObjectUndo(created, "Crear UI de inventario 2.2C");
