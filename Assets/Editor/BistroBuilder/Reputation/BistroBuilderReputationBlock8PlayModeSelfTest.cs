...
public static class BistroBuilderReputationBlock8PlayModeSelfTest
{
    ...
    private static BistroBuilderAdvancedCustomerHistorySnapshot originalAdvancedHistory;
    ...
    private static BistroBuilderAdvancedCustomerHistoryService advancedHistory;
    ...
    private static void Initialize(bool commandLine)
    {
        ...
        advancedHistory = Find<BistroBuilderAdvancedCustomerHistoryService>();
        ...
        if (reputation == null || tracking == null || relations == null ||
            finance == null || marketing == null || advancedHistory == null ||
            canonical == null ||
        ...
        bool historyValid = advancedHistory.ValidateConfiguration(out historyError);
        ...
        if (!trackingValid || !historyValid || !facadeValid || !screenValid)
        {
        ...
        string historyResetError = string.Empty;
        ...
        bool historyReset = advancedHistory.TryRestoreSnapshot(
            BistroBuilderAdvancedCustomerHistoryEngine.CreateEmptySnapshot(),
            out historyResetError);
        if (!reputationReset || !runtimeReset || !marketingReset || !historyReset)
        {
    ...
    private static void CompletePaidVisit(bool commandLine)
    {
        ...
        BistroBuilderAdvancedCustomerExperienceResult advancedExperience =
            tracking.LastAdvancedExperience;
        bool advancedExperienceValid = advancedExperience != null &&
            advancedExperience.individuals != null &&
            advancedExperience.individuals.Count == testGroup.GroupSize &&
            advancedExperience.aggregateSatisfactionBasisPoints ==
                tracking.LastRecordedSatisfactionBasisPoints;
        ...
        BistroBuilderReputationRuntimeSnapshot finalizedRuntime =
            tracking.CreateRuntimeSnapshot();
        bool finalizedMarkerValid = finalizedRuntime != null &&
            finalizedRuntime.visits != null && finalizedRuntime.visits.Count == 1 &&
            finalizedRuntime.visits[0] != null && finalizedRuntime.visits[0].finalized &&
            tracking.ActiveVisitCount == 0;
        if (!finalizedMarkerValid)
        {
            Finish(false,
                "8G/10C: reputation.runtime no conserva el marcador de visita finalizada.",
                commandLine);
            return;
        }

        testGroup.SetState(CustomerGroupState.Finished);
        BistroBuilderAdvancedCustomerHistorySnapshot historySnapshot =
            advancedHistory.CreateSnapshot();
        bool historyValid = historySnapshot != null &&
            historySnapshot.customers != null && historySnapshot.customers.Count == 1 &&
            historySnapshot.pendingOutcomes.Count == 0 &&
            historySnapshot.pendingAssignments.Count == 0 &&
            historySnapshot.customers[0].visitCount == 1 &&
            historySnapshot.customers[0].recentVisits.Count == 1;
        if (!historyValid || reputation.TotalExperiences != 1 ||
            reputation.ReviewCount != 1 || tracking.ActiveVisitCount != 0)
        {
            Finish(false,
                "8G/10C: finalización duplicada o historial inconsistente. Exp=" +
                reputation.TotalExperiences + " | Reviews=" + reputation.ReviewCount +
                " | Active=" + tracking.ActiveVisitCount + ".",
                commandLine);
            return;
        }
        ...
        Finish(true,
            ...
            "satisfacción individual -> reputación -> cohorte -> historial -> reseña -> " +
            "boca a boca -> UI.",
            commandLine);
    }
    ...
    private static void CaptureOriginalState()
    {
        ...
        originalAdvancedHistory = advancedHistory.CreateSnapshot();
    ...
    private static bool RestoreOriginalState(out string error)
    {
        ...
        if (originalAdvancedHistory != null &&
            !advancedHistory.TryRestoreSnapshot(originalAdvancedHistory, out error)) return false;
