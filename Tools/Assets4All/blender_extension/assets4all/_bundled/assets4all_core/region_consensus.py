...
def _evidence_items(evidence: BoundaryEvidence) -> Dict[str, float]:
    return {
...
def consensus_strength(
    ...
    variance = sum(weight * (value - mean) ** 2 for value, weight in weighted) / total_weight
    agreement = math.exp(-2.6 * variance)
    quorum = sum(1 for value, _ in weighted if value >= 0.58) / len(weighted)
    return _clamp01(mean * (0.62 + 0.23 * agreement + 0.15 * quorum))
...
def boundary_persistence(
    ...
    rng = random.Random(config.random_seed + int(evidence.edge_id) * 1009)
    base_items = _evidence_items(evidence)
    survived = 0
    runs = max(1, int(config.perturbation_runs))
    for _ in range(runs):
        perturbed = {
            name: _clamp01(value + rng.gauss(0.0, config.evidence_jitter))
        ...
        threshold = config.boundary_threshold + rng.uniform(
            -config.threshold_jitter,
            config.threshold_jitter,
        )
        if consensus_strength(synthetic, view_weights) >= threshold:
            survived += 1
...
def classify_boundary(
    ...
    persistence = boundary_persistence(evidence, config, view_weights)
    confidence = _clamp01(0.55 * persistence + 0.45 * strength)
    is_boundary = (
...
def classify_boundaries(
    ...
    return [classify_boundary(item, config, view_weights) for item in evidences]
